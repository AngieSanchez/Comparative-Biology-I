...
str(misdaticos)
str(misdaticos$U15717)

length(misdaticos$U15717) ### Longitud of the first sequences
length((misdaticos[[1]])) ### Another way. If the list don't have names, is necesary this way. 


lengthSeq <- lapply(X = misdaticos, FUN = length) # Give me the length of all sequences of the list.
lengthSeq

pseudoAling <- as.character(misdaticos) ##Convert from numbers to acgt
names<-attr(misdaticos, "species")

library("seqinr")


AllSeq <- write.fasta(sequences = pseudoAling,file.out = "datos.fas",names = names)

str(pseudoAling)

pseudoAling$U15717[3] #Extract only a base of all seq.
pseudoAling[[3]][3] ##Another way to extract only a base of all seq.

NewSeq <- as.data.frame(pseudoAling) 
str(NewSeq)

###Extract each position
position1 <- seq(1, 1045, by= 3)
position2 <- seq(2, 1045, by= 3)
position3 <- seq(3, 1045, by= 3)

by1<-NewSeq[position1,]
by2<-NewSeq[position2,]
by3<-NewSeq[position3,]

str(by1)

write.fasta(sequences = by1,file.out = "by1.fas",names = names)
write.fasta(sequences = by2,file.out = "by2.fas",names = names)
write.fasta(sequences = by3,file.out = "by3.fas",names = names)
